...
bool Dynamics::Init()
{
    ...
    fprintf(stderr, "[GetGravity] kdl_chain.getNrOfJoints() = %zu\n", (size_t)kdl_chain.getNrOfJoints());
...
bool Dynamics::InitFromXMLString(const std::string& xml)
{
    urdf_model_interface = urdf::parseURDF(xml);
    if (!urdf_model_interface) {
        fprintf(stderr, "Failed to parse URDF from XML string\n");
        return false;
    }

    if (!kdl_parser::treeFromUrdfModel(*urdf_model_interface, kdl_tree)) {
        fprintf(stderr, "Failed to extract KDL tree from XML string\n");
        return false;
    }
    ...
    fprintf(stderr, "[InitFromXMLString] kdl_chain.getNrOfJoints() = %zu\n",
            (size_t)kdl_chain.getNrOfJoints());
...
{
        fprintf(stderr, "[Dynamics] URDF: %s\n", urdf_path.c_str());
        fprintf(stderr, "[Dynamics] Chain: '%s' -> '%s'\n", start_link.c_str(), end_link.c_str());
        fprintf(stderr, "[Dynamics] DOF (KDL joints): %zu\n", (size_t)kdl_chain.getNrOfJoints());
        ...
        for (const auto &seg : kdl_chain.segments) {
                ...
                fprintf(stderr, "  [seg %zu] link='%s' joint='%s' type=%d inertial=%s%s\n",
                        seg_idx++, child_link.c_str(), j.getName().c_str(), j.getType(),
                        has_inertial ? "yes" : "NO",
                        has_inertial ? (", m=" + std::to_string(mass)).c_str() : "");
        }

        fprintf(stderr, "[Dynamics] Inertials present on %zu/%zu segments.\n",
                inertial_ok, kdl_chain.segments.size());
}
